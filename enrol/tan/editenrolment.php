...
require_capability("enrol/rk:manage", context_course::instance($course->id, MUST_EXIST));
...
$url = new moodle_url('/enrol/rk/editenrolment.php', $returnurl->params());
...
if (!$plugin->allow_manage($instance) || $instance->enrol != 'rk' || !($plugin instanceof enrol_self_plugin)) {
    print_error('erroreditenrolment', 'enrol');
}

// Get the self enrolment edit form.
$mform = new enrol_rk_user_enrolment_form($url, array('user'=>$user, 'course'=>$course, 'ue'=>$ue));
...
$title = get_string('editenrolment', 'enrol_rk');
