 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_tan_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_tan'), 'enrol_tan');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_tan'), 'enrol_tan');
        }
    ...
    public function add_course_navigation($instancesnode, stdClass $instance) {
        if ($instance->enrol !== 'tan') {
             throw new coding_exception('Invalid enrol instance type!');
        }

        $context = context_course::instance($instance->courseid);
        if (has_capability('enrol/tan:config', $context)) {
            $managelink = new moodle_url('/enrol/tan/edit.php', array('courseid'=>$instance->courseid, 'id'=>$instance->id));
    ...
    public function get_action_icons(stdClass $instance) {
        ...
        if (has_capability('enrol/tan:config', $context)) {
            $editlink = new moodle_url("/enrol/tan/edit.php", array('courseid'=>$instance->courseid, 'id'=>$instance->id));
    ...
    public function get_newinstance_link($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/tan:config', $context)) {
            return NULL;
        }
        // Multiple instances supported - different roles with different password.
        return new moodle_url('/enrol/tan/edit.php', array('courseid'=>$courseid));
    }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        require_once("$CFG->dirroot/enrol/tan/locallib.php");
        require_once("$CFG->dirroot/group/lib.php");

        $form = new enrol_tan_enrol_form(NULL, $instance);
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_tan', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_tan', format_string($course->fullname, true, array('context'=>$context)));
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/tan:manage')) {
    ...
    public function get_user_enrolment_actions(course_enrolment_manager $manager, $ue) {
        ...
        $params['ue'] = $ue->id;
        if ($this->allow_unenrol($instance) && has_capability("enrol/tan:unenrol", $context)) {
            $url = new moodle_url('/enrol/unenroluser.php', $params);
            $actions[] = new user_enrolment_action(new pix_icon('t/delete', ''), get_string('unenrol', 'enrol'), $url, array('class'=>'unenrollink', 'rel'=>$ue->id));
        }
        if ($this->allow_manage($instance) && has_capability("enrol/tan:manage", $context)) {
            $url = new moodle_url('/enrol/tan/editenrolment.php', $params);
...
function enrol_tan_print_header($options = array())
{
	global $CFG, $COURSE, $OUTPUT, $DB, $USER;

	$strheader = get_string('pluginname', 'enrol_tan');
	...
	print_header_simple(get_string('generator', 'enrol_tan'), '', $navigation, "", "", true);

	// header
	if (empty($options['noheading']))
		echo $OUTPUT->heading(get_string('generator', 'enrol_tan'));
}
